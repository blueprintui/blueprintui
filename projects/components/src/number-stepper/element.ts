 ...
 * @event {CustomEvent} increment - fires when increment button is clicked
 * @event {CustomEvent} decrement - fires when decrement button is clicked
 */
export class BpNumberStepper extends FormControl {
  ...
  /** hide native browser stepper arrows in the input field */
  @property({ type: Boolean, attribute: 'hide-stepper' }) accessor hideStepper = false;
  ...
  updated(props: PropertyValues<this>) {
    super.updated(props);
    if (props.has('hideStepper')) {
      this.style.setProperty('--hide-stepper', this.hideStepper ? 'none' : 'auto');
    }
  }
  ...
  #handleIncrement(e: Event) {
    ...
    if (!this.disabled && (this.max === undefined || this.valueAsNumber < this.max)) {
      this.stepUp();
      this.dispatchEvent(new CustomEvent('increment', { detail: { value: this.valueAsNumber } }));
    }
  }
  ...
  #handleDecrement(e: Event) {
    ...
    if (!this.disabled && (this.min === undefined || this.valueAsNumber > this.min)) {
      this.stepDown();
      this.dispatchEvent(new CustomEvent('decrement', { detail: { value: this.valueAsNumber } }));
    }
  }
