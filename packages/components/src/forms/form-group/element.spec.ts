...
describe('bp-form-group', () => {
  ...
  it('should sync layouts when a control overflows', async () => {
    ...
    formGroup.dispatchEvent(new CustomEvent('bpResizeChange', { detail: { width: 599 }, bubbles: true }));
    await elementIsStable(fields[1]);
    expect(fields[0].layout).toBe('horizontal-inline');

    formGroup.dispatchEvent(new CustomEvent('bpResizeChange', { detail: { width: 499 }, bubbles: true }));
    await elementIsStable(fields[1]);
    expect(fields[0].layout).toBe('horizontal');

    formGroup.dispatchEvent(new CustomEvent('bpResizeChange', { detail: { width: 399 }, bubbles: true }));
    await elementIsStable(fields[1]);
    expect(fields[0].layout).toBe('vertical-inline');

    formGroup.dispatchEvent(new CustomEvent('bpResizeChange', { detail: { width: 299 }, bubbles: true }));
    await elementIsStable(fields[1]);
    expect(fields[0].layout).toBe('vertical');

    formGroup.dispatchEvent(new CustomEvent('bpResizeChange', { detail: { width: 701 }, bubbles: true }));
    await elementIsStable(fields[1]);
    expect(fields[0].layout).toBe('compact');
  });
